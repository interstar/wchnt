...
```bash
lein live          # wiki → live/public/js/main.js
lein live-test     # semantics tests → live/public/js/tests.js + examples/
```

That is `lein with-profile +live cljsbuild once`. It writes `live/public/js/main.js` (and `tests.js` for `live-test`).
...
Opening `index.html` as `file://` may work in some browsers but storage is origin-specific and often unreliable. Use a static server for `tests.html` too so `fetch('live-examples/…')` works. Rebuild after changing `src/` or `live/src/`.
